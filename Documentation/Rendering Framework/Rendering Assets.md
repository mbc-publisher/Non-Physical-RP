...
NPRP’s Render pipeline asset. Must be set in the project's settings to take effect, as any Unity’s Render pipeline asset.
...
| Bicubic Rescaling | Enum | Off Up Only Up And Down | Whether the bicubic filtering is used at all, only when scaling back up or when scaling un and down. |
